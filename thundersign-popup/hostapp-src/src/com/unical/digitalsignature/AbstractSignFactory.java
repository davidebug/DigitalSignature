...
public abstract class AbstractSignFactory implements ISignFactory {
	...
	private static String className = "AbstractSignFactory";
	...
	public void showCertificateData(CertificateToken ct) {
		MiddlewareChrome.log(className,ct.getCertificate().toString());
	}
	...
	@Override
	public String createSignedFile(DSSDocument signedDocument) {
		String newfilename = getNameNewFile();

		String dir = getOutputDirectory();

		return writeFile(dir, newfilename, signedDocument);
	...
	public String writeFile(String dir, String newfilename, DSSDocument signedDocument) {
		MiddlewareChrome.log(className,"Create signed file: " + newfilename);
//		MiddlewareChrome.log(className,"Create signed file: " + Utility.buildFilePath(dir,newfilename));
		...
		return dir + Utility.separator + newfilename;
	}
	...
	private void format(String s, boolean val) {
		MiddlewareChrome.log(className," " + s + " => " + val);
	}
	...
	private void printCertificateSelectionField(DSSPrivateKeyEntry key, int index) {
		...
		MiddlewareChrome.log(className,"[" + index + "] - certificate:" + humanReadableSigner);
	}
