...
function shouldPrettyPrint(source) {
  if (!source || isPretty(source) || !isJavaScript(source) || isOriginal(source) || _prefs.prefs.clientSourceMapsEnabled && source.sourceMapURL) {
    return false;
  }
...
function getTextAtPosition(source, location) {
  ...
  if (source.isWasm) {
    ...
    const lines = (0, _wasm.renderWasmText)(source);
