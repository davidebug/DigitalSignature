...
function groupBreakpoints(breakpoints, selectedSource) {
  ...
  const map = (0, _lodash.groupBy)(breakpoints.filter(breakpoint => !breakpoint.options.hidden), breakpoint => (0, _sourceMaps.getSelectedLocation)(breakpoint, selectedSource).line);

  for (const line in map) {
    map[line] = (0, _lodash.groupBy)(map[line], breakpoint => (0, _sourceMaps.getSelectedLocation)(breakpoint, selectedSource).column);
  }
...
function filterByLineCount(positions, selectedSource) {
  ...
  for (const breakpoint of positions) {
    const { line } = (0, _sourceMaps.getSelectedLocation)(breakpoint, selectedSource);
    if (!lineCount[line]) {
      lineCount[line] = 0;
    }
  ...
  return positions.filter(breakpoint => lineCount[(0, _sourceMaps.getSelectedLocation)(breakpoint, selectedSource).line] > 1);
}

function filterVisible(positions, selectedSource, viewport) {
  return positions.filter(columnBreakpoint => {
    const location = (0, _sourceMaps.getSelectedLocation)(columnBreakpoint, selectedSource);
...
function filterByBreakpoints(positions, selectedSource, breakpointMap) {
  return positions.filter(position => {
    const location = (0, _sourceMaps.getSelectedLocation)(position, selectedSource);
...
function getFirstBreakpointPosition(state, { line, sourceId }) {
  ...
  return positions.find(position => (0, _sourceMaps.getSelectedLocation)(position, source).line == line);
}